...
class KeyOperateBrowser:
    ...
    # 当无法定位到单个元素时，就可以定位多个元素，返回多个或者选某一个
    def find_els(self, name, value):
        els = self.driver.find_elements(name, value)
        return els
    ...
    # 鼠标悬停操作封装，用于处理会变化的动态元素 轮播图等
    def mouse_hold_click(self, move_button_path, click_button):
        button_on = self.driver.find_element(
            By.XPATH, move_button_path
        )  # 不明白为什么用findelements 为了兼容性更好吗,我来改一下
        ...
        # 关键字封装的类函数里面不应该存在参数引用 如果封装的函数引用其他带参函数，
        # 是否需要在函数声明的时候把参数也加上，这种写法好吗？
        # 好像也没必要这么封装 因为已经有封装好的函数 再次调用就好了
        # 如果需要点击的刚好是悬停元素那其实可以封装进去 根据业务需要吧
    ...
    def send_click(self, name, value):
        self.find_el(name, value).click()

    # 封装一个ActionChains类 模拟按钮按下去效果 常规封装新建章节按钮点不动╮(╯▽╰)╭
    def button_key_down(self, name, value):
        el = self.find_el(name, value)
        ActionChains(self.driver).click(el)
